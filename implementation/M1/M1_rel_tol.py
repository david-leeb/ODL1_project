...
def main():
    ...
    output_file = "../results/M1/rel_tol_sweep_results.csv"
    
    # Tolerance values to test
    tolerance_values = [1e-2, 5e-3, 1e-3, 5e-4, 1e-4, 5e-5, 1e-5]
    
    # Number of trials for stability
    num_trials = 5
    
    # Fixed rho value (from optimal rho sweep)
    rho = 20
    
    # High max_iter to avoid limiting convergence
    max_iter = 10000
    
    print(f"Using device: {device}")
    print(f"Number of trials per tolerance: {num_trials}")
    ...
    for tol in tolerance_values:
        print(f"\n{'='*60}")
        print(f"Testing rel_tol = rel_tol2 = {tol}")
        print(f"{'='*60}")
